...
All tables live in a Supabase (Postgres) project. The base schema is in `supabase-schema.sql`; migrations in `supabase-migrations/` document incremental changes (latest here: `073`; all of `025`–`073` **applied in prod** — `072`–`073`, the security-review pair, applied 2026-08-27 right after their deploy; `025`–`070` as of 2026-07-09, `071` (group welcome notes, both parts) applied 2026-07-11 — `059` is the reserved/generated Clerk remap, applied at cutover; `065`, the destructive shifts-legacy drop, applied 2026-07-09 after its code deploy). Confirm `025` (column renames), `029` (the `application-files` bucket), `033` (group messaging), `034`/`035` (group icon images + `group-badges` bucket, `badge_image` → `icon_image`), `036`–`038` (public profile fields, members/profiles, member distinctions), `039`–`041` (lead-up gatherings + notify/image), `042`–`044` (group collections + per-collection profile visibility + per-collection self-join), `045`–`047` (shifts redesign + shift times), and `048`/`049` (participation leads + per-event lead opt-in; the gathering halves are dropped by `050`) are applied before relying on those features.

---

## Tables

> **Tenancy (migration `074`, branch 1a of [tenancy-design.md](tenancy-design.md)):** every table below except `push_tokens` and `notification_preferences` carries `community_id UUID NOT NULL REFERENCES communities(id)` (indexed `<table>_community_idx`). It is not repeated in each table's column list. Migration `075` (branch 1d) removed the transitional default and made the per-person unique constraints per-(community, person): `members (community_id, clerk_user_id)`, `camp_signups (community_id, clerk_user_id)`, `page_content` PK `(community_id, key)`, `attunement_nudges` PK `(community_id, clerk_user_id)`, `event_reminders_sent (community_id, clerk_user_id, target_date, phase)`, `conversations (community_id, direct_key)` partial unique, `shift_types (community_id, backfill_key)`, plus `applications (community_id, clerk_user_id)` partial unique. Code reaches scoped tables through `tenantDb(community.id)` (`lib/tenant-db.ts`). **RLS (migration `076`, branch 1e):** every table has row-level security on; scoped tables carry policy `tenant_isolation` (for `authenticated`: `community_id = request_community_id()`, the `community_id` claim of the request JWT), person-level tables carry `app_access` (`authenticated`, unconditional), `communities` is select-only for `authenticated`, and `anon` has no policies at all. The scoped client sends a per-community JWT as `authenticated` when `SUPABASE_JWT_SECRET` is set (`lib/tenant-token.ts`); the service-role key (which bypasses RLS) is used only by `lib/community.ts`, storage, scripts, and as the fallback when the secret is unset.
...
| `theme` | JSONB | `{}`. Read keys: `colors` = `{ <token>: "#rrggbb" }` overriding the `:root` colour tokens (`lib/theme.ts` — `ink`, `gold`, `purple`, `cream`, …), `fonts.display` = a font stack for the display face, `badge` = `{ base_url, font_url, width?, height?, font_name? }` for `/api/badge?c=<slug>` (absent → the repo's Glåüm assets). Invalid values are ignored. |
...
Also (`074`, **read since branch 1d**): `role` TEXT NOT NULL DEFAULT `'member'` (`member` / `admin` — community-scoped admin; `lib/admin-auth.ts` `requireAdmin()`; nothing reads Clerk `publicMetadata.role` any more) and `can_manage_polls` BOOL NOT NULL DEFAULT false (`lib/poll-auth.ts`). Set via `POST /api/admin/set-admin` / `set-poll-manager`; one-off backfill from the old Clerk flags: `node scripts/backfill-member-roles.mjs --execute [--owner=<email>]` (the `--owner` also stamps Clerk `publicMetadata.platformRole = 'owner'` — the platform owner is admin everywhere).
...
Many-to-many shift holds — a member **or active volunteer** (2026-07-16; same `clerk_user_id` key, no schema change) can sign up for **any number** of shift occurrences. **The single source of shift holds** (the legacy single `camp_signups.schedule_event_id` was dropped by `065`). Written by the `/api/shift-signups` API (POST/DELETE; gated on `getShiftParticipant`) and the admin `remove_shift`/`clear_shift`/`set_shift_role` actions; also **deleted** by `PATCH /api/admin/schedule/[id]` when a recurring event's `recurrence_days` is trimmed (rows whose `occurrence_date` falls outside the saved subset — the removed night's signups go with the night; the admin UI confirms the count first, 2026-07-16), **moved** by the same route's `split_night` action (save-for-just-one-night carves the night into its own one-off event; its signups follow — `schedule_event_id` → the new row, `occurrence_date` → NULL), and deleted by the event-delete CASCADE (the delete confirm counts the signups at stake). A member's held hours per shift type (summed `shiftDurationHours` of their held occurrences, `lib/shift-attunement.ts`) are what satisfy hour requirements.
...
- `catering_kitchen_state` / `catering_kitchen_state_test` — **Retired 2026-08-26, rows deleted 2026-08-27** (kitchen board extracted to the All Hands repo; state copied to the All Hands Supabase at cutover, then both rows dropped here once camp `/kitchen.html` + `/api/kitchen-list` were verified 404). These keys no longer exist in this database.
...
> **Tenancy (branch 1d):** every NEW upload goes to `<community_id>/…` inside its bucket (`objectPath()` in `lib/tenant-db.ts`): avatars `<cid>/<clerkUserId>/avatar.<ext>`, group/department/distinction icons `<cid>/<…>`, schedule icons and lead-up images `<cid>/<ts>-<rand>.<ext>`, application files `<cid>/<clerkUserId>/<ts>-<name>`. Objects uploaded before 1d keep their unprefixed paths (tables store full URLs); the private `application-files` read gate accepts both shapes and refuses a prefixed path from another community.
...
| `application-files` | Attachments for admin-added **File upload** application fields (`/api/apply/file`) | **Private** since migration `072` (created public by `029`) — reads go through GET `/api/apply/file?path=` (owner-or-admin → 60s signed URL); never rely on object URLs |
...
| `029_application_files_bucket.sql` | Public `application-files` storage bucket + read policy (for File-upload fields). **Must be applied** (or create the bucket manually). *The bucket is made private by `072`.* |
...
| `076_rls_tenant_isolation.sql` | **Multi-tenancy step 1e** ([tenancy-design.md](tenancy-design.md) decision #4). Enables RLS on every table; `tenant_isolation` policy on the 34 scoped tables keyed on `public.request_community_id()` (the JWT `community_id` claim); `app_access` on `push_tokens` / `notification_preferences`; select-only `app_read` on `communities`; grants schema/table/sequence use + `claim_shift_signup()` execute to `authenticated`; nothing for `anon`. **Apply BEFORE setting `SUPABASE_JWT_SECRET`** (without the secret the app keeps the service key; with the secret but without this migration every scoped query returns empty). Verify with `node scripts/verify-tenant-isolation.mjs`. Non-destructive, idempotent. |
| `075_tenancy_constraints.sql` | **Multi-tenancy step 1d** ([tenancy-design.md](tenancy-design.md) §1.3). Drops the `074` transitional column default + `platform_default_community_id()`; swaps the per-person unique constraints for per-(community, person) ones (members, camp_signups, page_content PK, attunement_nudges PK, event_reminders_sent, conversations direct_key index, shift_types backfill_key; adds a partial unique on `applications (community_id, clerk_user_id)`). Constraints are found by column set, not name. **Apply TOGETHER WITH the 1d deploy** — three upserts switch their ON CONFLICT target to the composite keys (camp_signups role signup, page_content admin save, attunement_nudges cron claim) and fail while code and schema disagree. Non-destructive, idempotent. |
| `074_communities.sql` | **Multi-tenancy step 1a** ([tenancy-design.md](tenancy-design.md)). `communities` table + Glåüm seed row; `community_id` (NOT NULL, FK, indexed) on all 34 community-scoped tables, backfilled to Glåüm, with the **transitional default** `platform_default_community_id()` (dropped in `075`); `members.role` + `members.can_manage_polls`; `claim_shift_signup()` re-created to stamp `community_id` from the event (same signature). Unique constraints untouched until `075`. **Applied in prod 2026-09-11** (before 1b merged — 1b's scoped queries need the column). **Either deploy order is safe** for 1a (old code: defaults fill the column; new code: `lib/community.ts` falls back to a synthetic default community and warns until the table exists). Apply before branch 1b starts. Non-destructive, idempotent. |
