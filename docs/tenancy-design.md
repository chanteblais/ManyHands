...
in feature code.** Branch 1d (`feat/tenancy-auth-crons`) built the same day: DB-backed roles +
backfill script, hourly per-community crons, storage prefixes, badge by slug, `/api/me/communities`
+ `/communities`, migration 075. **Phase 1 is complete once 075 is applied and the backfill has run.**
...
program) — no code change needed. **Domain 2026-09-12: `withmanyhands.ca`** — platform host and
platform sender are built (`lib/platform.ts`, `docs/domains.md` runbook). **Decided (option 1):
every community is a subdomain of the platform domain** — Glåüm moves to `glaum.withmanyhands.ca`
(no redirect from `camp.glaum.ca`), so Clerk needs no satellites (paid); the satellite code stays
dormant for a future client on its own domain. The DNS / Vercel / Clerk / Resend steps are
Chanté's, in the runbook's order. Vercel Pro only matters for per-community cron hours.** Drafted 2026-09-11 from a full inventory of the schema (36 tables), config layer,
...
| 2 | `feat/demo-seed` — **seed built 2026-09-12**; real tenant 2 pending | Seed script for the fictional demo community *Lantern Hollow* (the App Review / showcase asset; `--reset` = nightly reseed; `--organizer` = the demo login). Then the real tenant 2: Clerk primary → platform root, `camp.glaum.ca` satellite, host rows, per-community Resend sender. | Two communities live on one deployment (locally verified: `localhost` → Glåüm, `127.0.0.1` → Lantern Hollow). |
