...
- **Picker + directory**: `GET /api/me/communities` (the person's memberships × communities, with each community's origin), the public `GET /api/communities` (discoverable communities + whether applications are open — `lib/community-directory.ts`), and `/communities` (memberships or a sign-in prompt, then "Find a community").
